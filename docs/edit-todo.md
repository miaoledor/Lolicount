...
在线编辑多图层立绘/卡片主题，实时预览 SVG 渲染效果，编辑内容暂存 localStorage，支持导出为标准主题包。

## 已剔除的需求

以下需求因技术约束或不在本次范围，不列入任务：

- **文字弧度（arc text）**：`textpath.go` 中有 `ArcPath`/`RenderArcText` 工具函数，但 `TextLayer.Render` 未接入弧度参数，且文字层仅预览不保存，弧度编辑投入产出比低，暂不实现。
- **评论功能**：超出本次范围。
